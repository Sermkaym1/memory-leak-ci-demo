...
full-demo: ## 🚀 ПОЛНЫЙ ЗАПУСК ДЕМО (установка + тесты + отчет) - ГЛАВНАЯ КОМАНДА!
	@echo "$(GREEN)╔════════════════════════════════════════════════════════════╗$(NC)"
	@echo "$(GREEN)║           🚀 ЗАПУСК ПОЛНОГО ДЕМО ПРОЕКТА                  ║$(NC)"
	@echo "$(GREEN)╚════════════════════════════════════════════════════════════╝$(NC)"
	@echo ""
	@echo "$(YELLOW)Это займет ~15-20 минут (установка + тесты)$(NC)"
	@echo ""
...
quick-demo: ## ⚡ БЫСТРОЕ ДЕМО (только запуск сервисов + короткие тесты 5 мин)
	@echo "$(GREEN)╔════════════════════════════════════════════════════════════╗$(NC)"
	@echo "$(GREEN)║              ⚡ БЫСТРОЕ ДЕМО (5 минут)                     ║$(NC)"
	@echo "$(GREEN)╚════════════════════════════════════════════════════════════╝$(NC)"
	@make install
	@make build
	@make up
...
test-quick: ## ⚡ Запустить быстрые тесты (5 минут)
	@echo "$(YELLOW)⚡ Запуск быстрых тестов (5 минут)...$(NC)"
	@. $(VENV)/bin/activate && pytest tests/test_quick_demo.py -v -s --alluredir=$(ALLURE_RESULTS)
...
open-grafana: ## 📈 Открыть Grafana в браузере
	...
	@echo "Login: admin"
	@echo "Password: admin"
	@xdg-open http://localhost:3000 2>/dev/null || open http://localhost:3000 2>/dev/null || echo "Откройте вручную: http://localhost:3000"

open-prometheus: ## 📊 Открыть Prometheus в браузере
	@echo "$(BLUE)📊 Открытие Prometheus...$(NC)"
	@xdg-open http://localhost:9090 2>/dev/null || open http://localhost:9090 2>/dev/null || echo "Откройте вручную: http://localhost:9090"
